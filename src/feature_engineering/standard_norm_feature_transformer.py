...
from feature_utilities import identify_feature, transform_date
...
class StandardNormFeatureTransformer(BaseFeatureTransformer):
    ...
    def transform_object_features(self):
        for i in range(0, self._train_features.shape[1]):
            feature_type = identify_feature(self._train_features.iloc[:, i].append(self._test_features.iloc[:, i]))
