...
class StandardWithDatesFeatureTransformer(BaseFeatureTransformer):
    ...
    def add_dates(self):
        date_features = ['VAR_0073', 'VAR_0075', 'VAR_0176', 'VAR_0179', 'VAR_0204', 'VAR_0217']

        for i in range(0, len(date_features)):
            for j in range(i+1, len(date_features)):
                feature_name = '{}_{}'.format(date_features[i], date_features[j])
                self._train_features[feature_name] = self._train_features[date_features[i]] - self._train_features[date_features[j]]
                self._test_features[feature_name] = self._test_features[date_features[i]] - self._test_features[date_features[j]]
